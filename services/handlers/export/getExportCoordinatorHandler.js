...
export default async function getExportCoordinatorHandler(req, res) {
  return abilityValidator({
    ...
    next: async (req, res) => {
      ...
      const results = await prisma.coordinator.findMany({
        include: {
          Faculty: {
            include: {
              user: true,
      ...
      if (results && results.length > 0) {
        const data = results.map((result) => ({
          ...
          active: Boolean(result.ActiveCoordinator) ? "yes" : "no",
        ...
        archive.append(csvContent, { name: `coordinator.csv` });
      }
