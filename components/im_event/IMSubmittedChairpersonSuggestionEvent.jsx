...
export default function IMSubmittedChairpersonSuggestionEvent({ iMEvent }) {
  return (
    <li className='mb-10 ml-6'>
      ...
      <p className='mb-4 text-sm font-normal text-gray-500 dark:text-gray-400'>
        ...
        <span className='font-semibold text-green-400'>
          &quot;{iMEvent?.SubmittedChairpersonSuggestion?.IM?.title}&quot;
        </span>{" "}
        received suggestions from{" "}
        <span className='font-semibold text-CITLDarkBlue'>
          {
            iMEvent?.SubmittedChairpersonSuggestion?.ChairpersonSuggestion
              ?.SubmittedChairpersonReview?.ChairpersonReview?.Chairperson
              ?.Faculty?.user?.name
          }
        </span>
        .
