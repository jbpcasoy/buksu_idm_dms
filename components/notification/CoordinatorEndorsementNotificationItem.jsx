...
export default function CoordinatorEndorsementNotificationItem({
  ...
  return (
    <NotificationItemBase
      ...
      imgSrc={
        notificationData?.CoordinatorEndorsement?.Coordinator?.Faculty?.user
          ?.image
      }
      time={notificationData?.createdAt}
      icon={
        <div className='absolute flex items-center justify-center w-5 h-5 ml-6 -mt-5 bg-green-500  border border-white rounded-full dark:border-gray-800'>
          <svg
            fill='none'
    ...
    >
      New Coordinator Endorsement from{" "}
      ...
      {` on IM: "${notificationData?.CoordinatorEndorsement?.IM?.title}"`}
    </NotificationItemBase>
  );
}
