...
export default function CoordinatorSuggestionNotificationItem({
  notification,
}) {
  const {
  ...
  return (
    <NotificationItemBase
      ...
      href={`/im/${notificationData?.SubmittedCoordinatorSuggestion?.IM?.id}`}
      imgSrc={
        notificationData?.SubmittedCoordinatorSuggestion?.CoordinatorSuggestion
          ?.SubmittedCoordinatorReview?.CoordinatorReview?.Coordinator?.Faculty
          ?.user?.image
      }
    ...
    >
      New Coordinator Suggestion from{" "}
      <span className='font-semibold text-gray-900 dark:text-white'>
        {
          notificationData?.SubmittedCoordinatorSuggestion
            ?.CoordinatorSuggestion?.SubmittedCoordinatorReview
